...
const ProductInfo = (props) => {
  ...
  useEffect(() => {
    ...
    .then(data => localStorage.setItem(`${product.category}${props.id}`, JSON.stringify(data[0]))
    );
  }, []);

  const [thisProduct, setThisProduct] = useState(JSON.parse(localStorage.getItem(`${product.category}${props.id}`)));
  ...
  for (let i = 2; i < Object.keys(thisProduct).length; i++)  {
    ...
    if(keyy.substring(0,5) == 'About') {
      console.log(Object.values(thisProduct)[i])
      productAboutInfoRef.push(Object.values(thisProduct)[i])
    } else {
      productSpecificPropValueRef.push(<PropValueInfo key={i} prop={keyy} value={Object.values(thisProduct)[i]} />)
    }
  }
  ...
  function formatKey(keyy) {
    for (let i = 0; i < keyy.length; i++) {
      if (i==0) {
        console.log("first char before: " + keyy[i])
        keyy = replaceAt(keyy, 0, keyy.charAt(0).toUpperCase())
        console.log("first char after: " + keyy[i])
      }
      if (keyy[i] == '_') {
    ...
    console.log("keyy before returning: " + keyy  )
    return keyy
  }
  ...
  return (
    <div id='product-info-container'>
      ...
      <div id='product-info-rating-div' >
        ...
        <div>
          {product.review_count} ratings | Search this page
        </div>
      </div>
      <div id='ac-badge-wrapper'>
        <span className='a-declarative black-background'> Azamon's Choice </span>
      ...
      <div id='price-div'>
        ...
        <div id='original-price-div'>
          List Price: <div id='original-price-list-price-div'> ${product.list_price} </div> 
        </div>
      </div>
